...
- 角色扮演 system prompt（可直接粘贴到任何 LLM）
- 5+ 对白样本，覆盖关键情绪场景
- 三视图生图 prompt（正 / 侧 / 背）
- `consistency.yaml`（锁定画风、色板、负面提示词、种子，让每张图都像同一个人）
...
克隆或下载本仓库，将 `ai-avatar/` 目录放到 Claude skills 目录：
...
> **为什么视觉锚点要最先做？** 先锁画风 / 色板 / 负面提示 / 种子，是跨图一致性的单一最大影响因素。到第 5 步才决定风格，前面所有视觉描述都得重新校准。
...
- **生图执行**：本 skill 只产出 prompt，实际生图建议配合 `comfyui` 或 即梦 或 GPT-image 等生图工具或 Skill
